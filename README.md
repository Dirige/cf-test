...
一个基于 Cloudflare Workers 和本地 Go 服务器的域名测速系统，可以自动测试优选域名的速度并更新 DNS 记录。
...
- 📊 **结果共享** - 测速结果可通过 Cloudflare Worker 共享到云端
...
└── .github/workflows/       # GitHub Actions
    ├── build.yml            # 构建二进制
    ├── docker.yml           # 构建 Docker 镜像
    └── worker.yml           # 部署 Worker
```
...
# 编辑 wrangler.toml 填入你的 account_id 和 database_id
```

#### 3. 创建 D1 数据库

```bash
wrangler d1 create cf-test
...
```bash
wrangler d1 execute cf-test --file=schema.sql
```
...
# macOS ARM64
GOOS=darwin GOARCH=arm64 go build -o cfst-server .
```
...
1. **build.yml** - 编译多平台二进制文件
2. **docker.yml** - 构建并推送 Docker 镜像
3. **worker.yml** - 部署 Cloudflare Worker
...
```bash
docker pull yourusername/cfst-server:latest
docker run -d -p 8080:8080 -v ./config.yaml:/app/config.yaml yourusername/cfst-server:latest
```
...
```bash
cd cfst-server
docker build -t cfst-server .
docker run -d -p 8080:8080 -v ./config.yaml:/app/config.yaml cfst-server
```
